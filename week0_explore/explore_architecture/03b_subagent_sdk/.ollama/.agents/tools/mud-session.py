#!/usr/bin/env python3
"""Keep one authenticated CircleMUD socket alive and read commands on stdin."""
...
class MudError(RuntimeError):
    """A safe, user-facing session error."""


def load_project_env() -> None:
    """Load only MUD settings without placing secrets in process arguments."""
    allowed = {"MUD_USERNAME", "MUD_PASSWORD", "MUD_HOST", "MUD_PORT"}
    script_path = Path(__file__).resolve()
    repo_root = next((parent for parent in script_path.parents if (parent / ".git").exists()), None)
    if repo_root is None:
        raise MudError("could not locate the Git worktree")
...
class MudSession:
    ...
    def read_until_quiet(self, quiet: float = 1.0, timeout: float = 10.0) -> str:
        ...
        while time.monotonic() < deadline:
            ...
            if self.buffer and last_data_at is not None and time.monotonic() - last_data_at >= quiet:
                break
    ...
    def login(self, username: str, password: str) -> str:
        login_timeout = float(os.environ.get("MUD_LOGIN_TIMEOUT", "30"))
        deadline = time.monotonic() + login_timeout

        def remaining() -> float:
            available = deadline - time.monotonic()
            if available <= 0:
                raise MudError(f"login exceeded the {login_timeout:g}s overall timeout")
            return available

        print("MUD_LOGIN_STAGE=waiting_for_name_prompt", flush=True)
        self.read_until(re.compile(r"By what name do you wish to be known.*\?", re.I), remaining())
        print("MUD_LOGIN_STAGE=name_prompt_received", flush=True)
        self.send(username)
        self.read_until(re.compile(r"Password", re.I), remaining())
        print("MUD_LOGIN_STAGE=password_prompt_received", flush=True)
        self.send(password)
        result = self.read_until(re.compile(r"Welcome|Reconnecting|Wrong password", re.I), remaining())
        if re.search(r"Wrong password", result, re.I):
            raise MudError("wrong password")
        if re.search(r"Reconnecting", result, re.I):
            print("MUD_LOGIN_STAGE=reconnecting", flush=True)
            return self.read_until_prompt(min(remaining(), 10.0))

        print("MUD_LOGIN_STAGE=credentials_accepted", flush=True)
        self.send("")
        intro = self.read_until_quiet(quiet=0.35, timeout=min(remaining(), 4.0))
        # Some builds have another press-return page before the numbered menu.
        for _ in range(2):
            if re.search(r"Enter the game|Choice|Selection", intro, re.I):
                break
            if not re.search(r"press.*return|hit.*return|continue", intro, re.I):
                break
            self.send("")
            intro += self.read_until_quiet(quiet=0.35, timeout=min(remaining(), 4.0))
        print("MUD_LOGIN_STAGE=selecting_enter_game", flush=True)
        self.send("1")
        room = self.read_until_prompt(min(remaining(), 10.0))
        print("MUD_LOGIN_STAGE=playing", flush=True)
        return room
...
def main() -> int:
    try:
        ...
        session = MudSession(os.environ.get("MUD_HOST", "localhost"), int(os.environ.get("MUD_PORT", "4000")))
        print("MUD_LOGIN_STAGE=connecting", flush=True)
        session.open()
        try:
            room = session.login(username, password)
            print("MUD_LOGIN_OK", flush=True)
            if room.strip():
                print(room, flush=True)
            session.send("look")
            print(session.read_until_prompt(timeout=10.0), flush=True)
