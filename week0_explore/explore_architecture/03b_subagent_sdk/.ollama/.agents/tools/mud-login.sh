...
load_project_env() {
  ...
  # The SDK's two-player profiles override the default character only through
  # inherited process environment; credentials never appear in argv or output.
  if [[ -n "${MUD_LOGIN_USERNAME:-}" ]]; then
    MUD_USERNAME="$MUD_LOGIN_USERNAME"
  fi
  if [[ -n "${MUD_LOGIN_PASSWORD:-}" ]]; then
    MUD_PASSWORD="$MUD_LOGIN_PASSWORD"
  fi
}
