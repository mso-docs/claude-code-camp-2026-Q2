...
- **Mana:** 100(100) (live, via `score` and prompt bar "100M")
- **Moves:** 83(83) (live, via `score` and prompt bar "83V")
- **Armor Class:** 39/10 (live, via `score`)
- **Alignment:** 0 (live, via `score`)
- **Experience:** 1 exp, needs 1499 more for next level (live, via `score`)
- **Quest points:** 0 earned, 0 quests completed, not currently on a quest
  (live, via `score`)
- **Playing time:** 0 days, 6 hours (live, via `score`)
- **Condition:** hungry, thirsty, standing (live, via `score`)

## Attributes
- **Strength:** NOT YET OBSERVED (score does not show it; try a different
  command, e.g. `str`/`stats`/class-specific sheet, in a future session)
- **Intelligence:** NOT YET OBSERVED
- **Wisdom:** NOT YET OBSERVED
- **Dexterity:** NOT YET OBSERVED
- **Constitution:** NOT YET OBSERVED
- **Charisma:** NOT YET OBSERVED
- **Other server-reported stats:** see Vitals above (HP/Mana/Move/AC/
  alignment/exp/gold/questpoints/playtime/condition) - these ARE the stats
  CircleMUD's stock `score` command reveals; the six ability scores
...
Determine and live-confirm Smarty's age, gender, class, and stats while
